...
class RAGSystem:
    ...
    def _initialize_vectorstore(self):
        """Initialize or load existing vector store with proper permissions."""
        try:
            ...
            if os.path.exists(self.persist_directory):
                ...
                try:
                    ...
                    print(f"Warning: Existing database has permission issues. Recreating...")
                    # Force remove with retries
                    try:
                        shutil.rmtree(self.persist_directory)
                    except Exception:
                        # If rmtree fails, try harder
                        import time
                        time.sleep(0.5)
            ...
            # Close any existing vectorstore connection
            ...
            # Last resort: try with a completely new directory name
            import time
    ...
    def clear_collection(self):
        ...
        try:
            # Close the current vectorstore connection first
            if hasattr(self, 'vectorstore') and self.vectorstore is not None:
                try:
            ...
            if os.path.exists(self.persist_directory):
                ...
                try:
                    ...
                    # Try again with ignore_errors
                    time.sleep(0.5)
                    shutil.rmtree(self.persist_directory, ignore_errors=True)
                
                # Reinitialize with fresh database
                self._initialize_vectorstore()
                return "Collection cleared and reinitialized successfully"
            else:
                # Even if directory doesn't exist, reinitialize to ensure clean state
                self._initialize_vectorstore()
                return "No collection to clear, initialized fresh database"
                
        except Exception as e:
            print(f"Error during clear_collection: {e}")
            # Force reinitialize anyway
            self._initialize_vectorstore()
            return f"Collection cleared with warnings: {e}"
